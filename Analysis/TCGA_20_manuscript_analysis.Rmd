...
clinicals<-t(read.delim('TCGA20_clinical_data_ordered_all_clinical_variables_samples_as_columns.txt',sep='\t',header=1, row.names=1,check.names=F))# download this form GEO Accession # GSE62944
...
##cv across all genes
par( mfrow = c( 1,2 ) ,lwd=4)
com_counts_rsem<-rsem_her2_expected_counts[rownames(rsem_her2_expected_counts)%in%rownames(feature),]
hist(apply(com_counts_rsem[,1:12],1,sd)/apply(com_counts_rsem[,1:12],1,mean),main = "TCGA expected counts",xlab = "Coefficient of variation",xlim=c(0,4.5),ylim=c(0,8000),lwd=4,ylab="Number of genes")
abline(v=median(na.omit(apply(com_counts_rsem[,1:12],1,sd)/apply(com_counts_rsem[,1:12],1,mean))),lty=2,col='red')
hist(apply(com_counts_rsem[,13:17],1,sd)/apply(com_counts_rsem[,13:17],1,mean),main = "TCGA expected counts",xlab = "Coefficient of variation",xlim=c(0,4.5),ylim=c(0,4000),lwd=4,ylab="Number of genes")
abline(v=median(na.omit(apply(com_counts_rsem[,13:17],1,sd)/apply(com_counts_rsem[,13:17],1,mean))),lty=2,col='red')


median(na.omit(apply(com_counts_rsem[,1:12],1,sd)/apply(com_counts_rsem[,1:12],1,mean)))
median(na.omit(apply(com_counts_rsem[,13:17],1,sd)/apply(com_counts_rsem[,13:17],1,mean)))
com_counts_rsubred<-feature[rownames(feature)%in%rownames(com_counts_rsem),]
hist(apply(com_counts_rsubred[,1:12],1,sd)/apply(com_counts_rsubred[,1:12],1,mean),main = "Rsubread gene counts",xlab = "Coefficient of variation",xlim=c(0,4.5),ylim=c(0,10000),lwd=4,ylab="Number of genes")
abline(v=median(na.omit(apply(com_counts_rsubred[,1:12],1,sd)/apply(com_counts_rsubred[,1:12],1,mean))),lty=2,col='red')
hist(apply(com_counts_rsubred[,13:17],1,sd)/apply(com_counts_rsubred[,13:17],1,mean),main = "Rsubread gene counts",xlab = "Coefficient of variation",xlim=c(0,4.5),ylim=c(0,10000),lwd=4,ylab="Number of genes")
abline(v=median(na.omit(apply(com_counts_rsubred[,1:12],1,sd)/apply(com_counts_rsubred[,1:12],1,mean))),lty=2,col='red')

median(na.omit(apply(com_counts_rsubred[,1:12],1,sd)/apply(com_counts_rsubred[,1:12],1,mean)))
median(na.omit(apply(com_counts_rsubred[,13:17],1,sd)/apply(com_counts_rsubred[,13:17],1,mean)))
print("Using TCGA expected gene counts")
print(paste("Total number of genes having coefficient of variation less than or equal to 1 ",sum(na.omit(apply(com_counts_rsem,1,sd)/apply(com_counts_rsem,1,mean)<=0.5)), sep=':'))
print(paste("Total number of genes having coefficient of variation more than 1 ",sum(na.omit(apply(com_counts_rsem,1,sd)/apply(com_counts_rsem,1,mean)>0.5)), sep=':'))

print("Using Rsubread gene counts")
print(paste("Total number of genes having coefficient of variation less than or equal to 1 ",sum(na.omit(apply(com_counts_rsubred,1,sd)/apply(com_counts_rsubred,1,mean)<=0.5)), sep=':'))
print(paste("Total number of genes having coefficient of variation more than 1 ",sum(na.omit(apply(com_counts_rsubred,1,sd)/apply(com_counts_rsubred,1,mean)>0.5)), sep=':'))
...
median(na.omit(apply(com_genes_TCGA[,1:12],1,sd)/apply(com_genes_TCGA[,1:12],1,mean)))
...
median(na.omit(apply(com_genes_TCGA[,1:12],1,sd)/apply(com_genes_TCGA[,1:12],1,mean)))
median(na.omit(apply(com_genes_TCGA[,13:17],1,sd)/apply(com_genes_TCGA[,13:17],1,mean)))
...
median((na.omit(apply(com_genes_fpkm[,13:17],1,sd)/apply(com_genes_fpkm[,13:17],1,mean))))
median((na.omit(apply(com_genes_fpkm[,1:12],1,sd)/apply(com_genes_fpkm[,1:12],1,mean))))
...
median((na.omit(apply(com_genes_tpm[,1:12],1,sd)/apply(com_genes_tpm[,1:12],1,mean))))
median((na.omit(apply(com_genes_tpm[,13:17],1,sd)/apply(com_genes_tpm[,13:17],1,mean))))
...
par(mfrow = c(1, 1))
boxplot(ihc_pos$Rsem_log_q_200_f,ihc_neg$Rsem_log_q_200_f,ihc_pos$FPKM_log_q_200_f,ihc_neg$FPKM_log_q_200_f,ihc_pos$TPM_log_q_200_f,ihc_neg$TPM_log_q_200_f,cex.axis=1.5, cex.lab=1.5,outpch=NA,range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,col='grey75',medcol="grey35",lwd=4,border = "grey35", ylab="Rank-based estimate of HER2 activation")
names=c("TCGA\nLevel3\nHER2(+)","TCGA\nLevel3\nHER2(-)","Rsubred\nFPKM\nHER2(+)", "Rsubred\nFPKM\nHER2(-)","Rsubred\nTPM\nHER2(+)", "Rsubred\nTPM\nHER2(-)")
text(seq(1,6,by=1),par("usr")[3] - 2.5, labels = names, srt = 45, pos = 1, xpd = TRUE)
...
Finally, we are assessing the classification predictions using 10-fold cross validation  and using TCGA Level 3 and Rsubread TPM normalized lung adenocarcinoma and lung squamous carcinoma samples.
```{r}
par(mfrow = c(1, 1),lwd=4)
