...
rsem_her2_expected_counts_f<-rsem_her2_expected_counts[,c(1:11,13,15,17)]
head(rsem_her2_expected_counts_f)
ecdf_all_ex_f<-apply(log2(rsem_her2_expected_counts_f+1),2,ecdf)

plot(ecdf_all_ex_f[[1]],xlab="log2(Total mapped reads)",ylab="Cumulative proportion",col="blue",main="TCGA pipeline",ylim=c(0,1),xlim = c(0,20),cex.axis=1.5, cex.lab=1.5)
legend(10,10,c("GFP", "HER2"), col = c("blue","brown"))
for(i in 2:11){lines(ecdf_all_ex_f[[i]],xlab=NA, ylab = NA,col="blue")}
for(i in 12:14){lines(ecdf_all_ex_f[[i]],xlab=NA, ylab = NA,col="brown")}
...
###if outlier samples were removed
feature_f<-feature[,c(1:11,13,15,17)]
head(feature_f)
ecdf_f<-apply(log2(feature_f+1),2,ecdf)

plot(ecdf_f[[1]],xlab="log2(Total mapped reads)",ylab="Cumulative proportion",col="blue",main="Rsubread pipeline",ylim=c(0,1),xlim = c(0,20),cex.axis=1.5, cex.lab=1.5)
legend(10,10,c("GFP", "HER2"), col = c("blue","brown"))
for(i in 2:11){lines(ecdf_f[[i]],xlab=NA, ylab = NA,col="blue")}
for(i in 12:14){lines(ecdf_f[[i]],xlab=NA, ylab = NA,col="brown")}
...
expected_counts<-apply(rsem_her2_expected_counts_f,2,sum)
feature_counts<-apply(feature_f,2,sum)
# Creating a plot showing total mapped reads per sample  
par( mfrow = c( 1, 2 ),lwd=4 )
x = c(rep(1, 11), rep(2, 3)) # this indicates where on the x axis to plot
par(mar=c(3.1, 4.6, 2.1, 0.6)) # figure margins

boxplot(log2(expected_counts[1:11]+1), log2(expected_counts[12:14]+1),range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,lwd=4,ylim=c(20,25),xlab="", ylab="log2(Total mapped reads)",main="TCGA Pipeline",col='grey75',medcol="grey75",lwd=4,border = "grey35")

points(jitter(x, factor=2), c(log2(expected_counts[1:11]+1), log2(expected_counts[12:14]+1)), pch=4, cex=2, col=1,  xaxt="n",cex.lab=1.5)
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5)

boxplot(log2(feature_counts[1:11]+1), log2(feature_counts[12:14]+1),range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,lwd=4,ylim=c(20,25),xlab="", ylab="log2(Total mapped reads)",col='grey75',medcol="grey75",lwd=4,main="Rsubread Pipeline",border = "grey35")
points(jitter(x, factor=1.5), c(log2(feature_counts[1:11]+1), log2(feature_counts[12:14]+1)), pch=4,cex=2,cex.lab=1.5,col="black")
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) 
...
boxplot(log2(rsem_her2$ERBB2[1:12]+1),log2(rsem_her2$ERBB2[13:15]+1),log2(rsub_her2$ERBB2[1:12]+1),log2(rsub_her2$ERBB2[13:15]+1),ylab="",range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,col='grey75',medcol="grey75",lwd=4,main=paste('Comparing TGCA and Rsubread Pipelines','\n', 'in Differentiating HER2 Overexpression from Controls',sep=''),border = "grey35")
...
##using data processed by RSEM detected difference in her2 gene count in HER2 overexpressed versus GFP overexpressed samples
##t = -12.1833, df = 4.157, p-value = 0.0002081 but was worse than Rsubread
t.test(log2(rsem_her2$ERBB2[1:11]+1),log2(rsem_her2$ERBB2[12:14]+1))

##using not normalized data processed by Rsubread was much better at detecting difference in her2 gene count in HER2 overexpressed versus GFP overexpressed samples
##t = -46.6747, df = 8.35, p-value = 2.152e-11
t.test(log2(rsub_her2$ERBB2[1:11]+1),log2(rsub_her2$ERBB2[12:14]+1))
...
standardized_mean(m.1=mean((log2(rsem_her2$ERBB2[13:15]+1))),sd.1=sd((log2(rsem_her2$ERBB2[13:15]+1))),n.1=3,m.2=mean((log2(rsem_her2$ERBB2[1:12]+1))),sd.2=sd((log2(rsem_her2$ERBB2[1:12]+1))),n.2 = 12)
####Hedge's standardized mean/effect size using Rsubread pipeline 
...
###if outlier samples are removed
##for TCGA pipeline
standardized_mean(m.1=mean((log2(tcga_her2_normalized$ERBB2[c(13,15,17)]+1))),sd.1=sd((log2(tcga_her2_normalized$ERBB2[c(13,15,17)]+1))),n.1=3,m.2=mean((log2(tcga_her2_normalized$ERBB2[1:12]+1))),sd.2=sd((log2(tcga_her2_normalized$ERBB2[1:12]+1))),n.2=12)
##for RSUBREAD FPKM
standardized_mean(mean(rsub_fpkmlog_her2$ERBB2[c(13,15,17)]),sd(rsub_fpkmlog_her2$ERBB2[c(13,15,17)]),3,mean(rsub_fpkmlog_her2$ERBB2[1:12]),sd(rsub_fpkmlog_her2$ERBB2[1:12]),12)
##for RSUBREAD TPM
standardized_mean(mean(rsub_tpmlog_her2$ERBB2[c(13,15,17)]),sd(rsub_tpmlog_her2$ERBB2[c(13,15,17)]),3,mean(rsub_tpmlog_her2$ERBB2[1:12]),sd(rsub_fpkmlog_her2$ERBB2[1:12]),12)
...
print(paste("Standardized mean difference in prediction between HER2 (+) and HER2 (-) samples for TCGA Level 3 data :",standardized_mean(m.1=mean(ihc_pos_t$Rsem_log_q_200_f),sd.1=sd(ihc_pos_t$Rsem_log_q_200_f),n.1=length(ihc_pos_t$Rsem_log_q_200_f),m.2=mean(ihc_neg_t$Rsem_log_q_200_f),sd.2=sd(ihc_neg_t$Rsem_log_q_200_f),n.2=length(ihc_neg_t$Rsem_log_q_200_f)),sep=' '))
...
##in TCGA BRCA samples####
setwd("~/Dropbox/TCGA_RNASeq_Clinical/Analysis_datasets/")
rsub_preds<-read.table("Rsub_5_01.txt", sep='\t', header=1, row.names=1)
tcga_preds<-read.table("Rsem_5_01.txt", sep='\t', header=1, row.names=1)
...
##coefficient of variation in TCGA pipeline processed HER2 predictions
print(paste("Coefficient of variation in TCGA pipeline processed HER2 predictions in HER2(-) BRCA samples",sd(ihc_neg_t$RSEM_preds)/mean(ihc_neg_t$RSEM_preds),sep=' '))
print(paste("Coefficient of variation in TCGA pipeline processed HER2 predictions in HER2(+) BRCA samples",sd(ihc_pos_t$RSEM_preds)/mean(ihc_pos_t$RSEM_preds),sep=' '))

##coefficient of variation in Rsubread FPKM pipeline processed HER2 predictions
print(paste("Coefficient of variation in Rsubread FPKM processed HER2 predictions in HER2(-) BRCA samples",sd(ihc_neg_t$RSUB_FPKM)/mean(ihc_neg_t$RSUB_FPKM),sep="  "))
print(paste("Coefficient of variation in Rsubread FPKM processed HER2 predictions in HER2(+) BRCA samples",sd(ihc_pos_t$RSUB_FPKM)/mean(ihc_pos_t$RSUB_TPM),sep="  "))
##coefficient of variation in Rsubread TPM pipeline processed HER2 predictions
print(paste("Coefficient of variation in Rsubread TPM  processed HER2 predictions in HER2(-) BRCA samples",sd(ihc_neg_t$RSUB_TPM)/mean(ihc_neg_t$RSUB_TPM),sep="  "))
print(paste("Coefficient of variation in Rsubread TPM processed HER2 predictions in HER2(+) BRCA samples",sd(ihc_pos_t$RSUB_TPM)/mean(ihc_pos_t$RSUB_TPM),sep="  "))

##Calculating standardized mean differences between the HER2(+) and HER2(-) groups
print(paste("Standardized mean difference in prediction between HER2 (+) and HER2 (-) samples for TCGA Level 3 data :",standardized_mean(m.1=mean(ihc_pos_t$RSEM_preds),sd.1=sd(ihc_pos_t$RSEM_preds),n.1=length(ihc_pos_t$RSEM_preds),m.2=mean(ihc_neg_t$RSEM_preds),sd.2=sd(ihc_neg_t$RSEM_preds),n.2=length(ihc_neg_t$RSEM_preds)),sep=' '))

print(paste("Standardized mean difference in prediction between HER2 (+) and HER2 (-) samples for Rsubread FPKM data :",standardized_mean(m.1=mean(ihc_pos_t$RSUB_FPKM),sd.1=sd(ihc_pos_t$RSUB_FPKM),n.1=length(ihc_pos_t$RSUB_FPKM),m.2=mean(ihc_neg_t$RSUB_FPKM),sd.2=sd(ihc_neg_t$RSUB_FPKM),n.2=length(ihc_neg_t$RSUB_FPKM)),sep=' '))

print(paste("Standardized mean difference in prediction between HER2 (+) and HER2 (-) samples for TPM data :",standardized_mean(m.1=mean(ihc_pos_t$RSUB_TPM),sd.1=sd(ihc_pos_t$RSUB_TPM),n.1=length(ihc_pos_t$RSUB_TPM),m.2=mean(ihc_neg_t$RSUB_TPM),sd.2=sd(ihc_neg_t$RSUB_TPM),n.2=length(ihc_neg_t$RSUB_TPM)),sep=' '))

## t-tests comparing HER(+) and HER(-) prediction
t.test(ihc_pos_t$RSEM_preds,ihc_neg_t$RSEM_preds)# For TCGA Level 3: p-value = 2.009e-05
t.test(ihc_pos_t$RSUB_FPKM,ihc_neg_t$RSUB_FPKM)#For Rsubread FPKM: p-value = 1.493e-10
t.test(ihc_pos_t$RSUB_TPM,ihc_neg_t$RSUB_TPM)#For Rsubread TPM:p-value = 3.197e-12
```
