...
hist(apply(com_counts_rsem,1,sd)/apply(com_counts_rsem,1,mean),main = "TCGA expected counts",xlab = "Coefficient of variation",xlim=c(0,4.5),ylim=c(0,10000),lwd=4,ylab="Number of genes")
com_counts_rsubred<-feature[rownames(feature)%in%rownames(com_counts_rsem),]
hist(apply(feature,1,sd)/apply(feature,1,mean),main = "Rsubread gene counts",xlab = "Coefficient of variation",xlim=c(0,4.5),ylim=c(0,10000),lwd=4,ylab="Number of genes")
print("Using TCGA expected gene counts")
print(paste("Total number of genes having coefficient of variation less than or equal to 1 ",sum(na.omit(apply(com_counts_rsem,1,sd)/apply(com_counts_rsem,1,mean)<=1)), sep=':'))
print(paste("Total number of genes having coefficient of variation more than 1 ",sum(na.omit(apply(com_counts_rsem,1,sd)/apply(com_counts_rsem,1,mean)>1)), sep=':'))

print("Using Rsubread gene counts")
print(paste("Total number of genes having coefficient of variation less than or equal to 1 ",sum(na.omit(apply(com_counts_rsubred,1,sd)/apply(com_counts_rsubred,1,mean)<=1)), sep=':'))
print(paste("Total number of genes having coefficient of variation more than 1 ",sum(na.omit(apply(com_counts_rsubred,1,sd)/apply(com_counts_rsubred,1,mean)>1)), sep=':'))
