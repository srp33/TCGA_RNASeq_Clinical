...
* Manually download the compiled and processed TCGA clinical data from GEO. Accession number is GSE62944
...
plot(jitter(x, factor=2,amount = 0.1), c(rsem_her2_expected_counts["ERBB2",1:12], rsem_her2_expected_counts["ERBB2",13:17]), pch=20, cex=2, col=x+1,ylim=c(0,200000), xlab="", ylab="Total Mapped Reads", xaxt="n", cex.axis=1.5, cex.lab=1.5, main="TCGA Level 3")
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) # this gives us more control over the axis
plot(jitter(x, factor=2), c(feature["ERBB2",1:12], feature["ERBB2",13:17]), pch=20, cex=2, col=x+1, xlab="", ylab="Total Mapped Reads", ylim=c(0,200000),xaxt="n", cex.axis=1.5, cex.lab=1.5, main="Rsubread")
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) # this gives us more control over the axis
...
par( mfrow = c( 2, 1 ) )
plot(ecdf_all_ex[[1]],xlab=NA, ylab = NA,col="green",main="TCGA pipeline")
...
par(mfrow = c(1,1))
plot(ecdf_all[[1]],xlab=NA, ylab = NA,col="green",main="Rsubread pipeline")
for(i in 2:12){lines(ecdf_all[[i]],xlab=NA,ylab = NA,col="green")}
for(i in 13:17){lines(ecdf_all[[i]],xlab=NA,ylab = NA,col="red")}
#title(main = "Emperical CDF with Rsubread pipeline for each samples")
dev.off()
...
# 
# text(seq(1,4,by=1),par("usr")[3] - 0.6, labels = names, srt = 45, pos = 1, xpd = TRUE)
...
Now, let's compare standardized mean differences, compute_dgs(n.1 = 5,m.1 = mean(rsem_her2$TCGA_ERBB2[13:17]),sd.1 = sd(rsem_her2$TCGA_ERBB2[13:17]),n.2 =12, m.2 =  mean(rsem_her2$TCGA_ERBB2[1:12]),sd.2 = sd(rsem_her2$TCGA_ERBB2[1:12]),data=rsem_her2)
t-test and coefficient of variation for the gene counts from two pipelines.
...
Standardized differences between HER2 activated and control samples using TCGA pipeline `r (compute_dgs(n.1 = 5,m.1 = mean((log2(rsub_her2$Rsubread_ERBB2[13:17]))),sd.1 = sd((log2(rsub_her2$Rsubread_ERBB2[13:17]))),n.2 =12, m.2 =  mean((log2(rsub_her2$Rsubread_ERBB2[1:12]))),sd.2 = sd((log2(rsub_her2$Rsubread_ERBB2[1:12]))),data=rsub_her2)$d)[1]` and using Rsubread pipeline `r (compute_dgs(n.1 = 5,m.1 = mean(rsem_her2$TCGA_ERBB2[13:17]),sd.1 = sd(rsem_her2$TCGA_ERBB2[13:17]),n.2 =12, m.2 =  mean(rsem_her2$TCGA_ERBB2[1:12]),sd.2 = sd(rsem_her2$TCGA_ERBB2[1:12]),data=rsem_her2)$d)[1]`
...
#compute_dgs(n.1 = 5,m.1 = mean((log2(tcga_her2_normalized$ERBB2[13:17]))),sd.1 = sd((log2(tcga_her2_normalized$ERBB2[13:17]))),n.2 =12, m.2 =  mean((log2(tcga_her2_normalized$ERBB2[1:12]))),sd.2 = sd((log2(tcga_her2_normalized$ERBB2[1:12]))),data=rsub_her2)#standardized difference is denoted by d.and it is 25.78 for TCGA pipeline processed data
...
#compute_dgs(n.1 = 5,m.1 = mean(rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[13:17]),sd.1 = sd(rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[13:17]),n.2 =12, m.2 =  mean(rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[1:12]),sd.2 = sd(rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[1:12]),data=rsem_her2)#standardized difference is denoted by d.and it is 66.92 for rsubread processed fpkmlog data
...
#compute_dgs(n.1 = 5,m.1 = mean(rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[13:17]),sd.1 = sd(rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[13:17]),n.2 =12, m.2 =  mean(rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[1:12]),sd.2 = sd(rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[1:12]),data=rsub_tpmlog_her2)#standardized difference is denoted by d.and it is 59.797 for rsubread processed tpmlog data
# 
# 
# png("normalized_expression.png",    # create PNG for the heat map        
...
# names=c("TCGA\nGFP","TCGA\nHER2","Rsubred FPKM\nGFP", "Rsubred FPKM\nHER2","Rsubred TPM\nGFP", "Rsubred TPM\nHER2")
# text(seq(1,6,by=1),par("usr")[3] - 1.5, labels = names, srt = 45, pos = 1, xpd = TRUE)
...
#par(mar=c(3.1, 4.6, 2.1, 0.6)) # figure margins
x_jit<-jitter(x, factor=6,amount = 0.1)
...
mean(ihc_pos$Rsem_log_q_200_f)-mean(ihc_neg$Rsem_log_q_200_f)
mean(ihc_pos$FPKM_log_q_200_f)-mean(ihc_neg$FPKM_log_q_200_f)
mean(ihc_pos$TPM_log_q_100_f)-mean(ihc_neg$TPM_log_q_100_f)
```
...
```{r}
##############violin plot: combination of boxplot and kernel density plot of HER2 pathway activity
##in TCGA BRCA samples####

par(mfrow = c(1, 1))
boxplot(ihc_pos$Rsem_log_q_200_f,ihc_neg$Rsem_log_q_200_f,ihc_pos$FPKM_log_q_200_f,ihc_neg$FPKM_log_q_200_f,ihc_pos$TPM_log_q_100_f,ihc_neg$TPM_log_q_100_f, col='grey', names=c("TCGA HER2(+)","TCGA HER2(-)","FPKM HER2(+)","FPKM HER2(-)","TPM HER2(+)","TPM HER2(-)"))
...
vioplot(ihc_pos$Rsem_log_q_200_f,ihc_neg$Rsem_log_q_200_f,col='grey', names=c("HER2(+)","HER2(-)"))
title("TCGA RSEM log Pipeline")
mean(ihc_pos$Rsem_log_q_200_f)-mean(ihc_neg$Rsem_log_q_200_f)#mean difference for TCGA level 3 pipeline

vioplot(ihc_pos$FPKM_log_q_200_f,ihc_neg$FPKM_log_q_200_f,col='grey', names=c("HER2(+)","HER2(-)"))
title("Rsubread FPKM log Pipeline")
mean(ihc_pos$FPKM_log_q_200_f)-mean(ihc_neg$FPKM_log_q_200_f)#mean difference for Rsubread pipeline FPKM data

vioplot(ihc_pos$TPM_log_q_100_f,ihc_neg$TPM_log_q_100_f,col='grey', names=c("HER2(+)","HER2(-)"))
title("Rsubread TPM log Pipeline")
mean(ihc_pos$TPM_log_q_100_f)-mean(ihc_neg$TPM_log_q_100_f) #mean difference for Rsubread pipeline TPM data
...
t.test(ihc_pos_t$TPM_log_q_100_f,ihc_neg_t$TPM_log_q_100_f)#p-value = 3.197e-12
