...
#rsem_her2_expected_counts<-log2(read.table("GFP18_HER2_TCGA_Pipeline_Expected_Gene_Counts.txt", sep='\t', header=1, row.names=1, check.names=F)+1)
...
```
Then we calculated standardized mean differences and t-test for the gene counts from two pipelines.
...
#compute_dgs(n.1 = 5,m.1 = mean(log2(rsem_her2$TCGA_ERBB2[13:17]+1)),sd.1 = sd(log2(rsem_her2$TCGA_ERBB2[13:17]+1)),n.2 =12, m.2 =  mean(log2(rsem_her2$TCGA_ERBB2[1:12]+1)),sd.2 = sd(log2(rsem_her2$TCGA_ERBB2[1:12]+1)),data=rsem_her2)#standardized difference is denoted by d.and it is 3.573901 for rsubread processed data
...
compute_dgs(n.1 = 5,m.1 = mean((log2(rsub_her2$Rsubread_ERBB2[13:17]+1))),sd.1 = sd((log2(rsub_her2$Rsubread_ERBB2[13:17]+1))),n.2 =12, m.2 =  mean((log2(rsub_her2$Rsubread_ERBB2[1:12]+1))),sd.2 = sd((log2(rsub_her2$Rsubread_ERBB2[1:12]+1))),data=rsub_her2)
```
Boxplot of ERBB2 gene counts from TCGA and Rsubread pipeline right after read alignment
...
points(jitter(rep(1,12),factor=1.5),log2(rsem_her2$TCGA_ERBB2[1:12]+1),pch=4,cex=2,cex.lab=1.5,col="black")
points(jitter(rep(2,5),factor=1.5),log2(rsem_her2$TCGA_ERBB2[13:17]+1),pch=4,cex=2,cex.lab=1.5,col='black')
points(jitter(rep(3,12),factor=1.5),log2(rsub_her2$Rsubread_ERBB2[1:12]+1),pch=4,cex=2,cex.lab=1.5,col='black')
points(jitter(rep(4,5),factor=1.5),log2(rsub_her2$Rsubread_ERBB2[13:17]+1),pch=4,cex=2,cex.lab=1.5,col='black')
...
boxplot(log2(expected_counts[1:12]+1), log2(expected_counts[13:17]+1),range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,col='grey',medcol="grey",lwd=3,ylim=c(20,25),xlab="", ylab="log2(Total mapped reads)",main="TCGA Level 3",lwd=4)
points(jitter(x, factor=1.5), c(log2(expected_counts[1:12]+1), log2(expected_counts[13:17]+1)), pch=20, cex=2.5, col=x+1,  xaxt="n", cex.axis=1.5, cex.lab=1.5)
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) # this gives us more control over the axis

boxplot(log2(feature_counts[1:12]+1), log2(feature_counts[13:17]+1),range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,col='grey',medcol="grey",lwd=3,ylim=c(20,25),xlab="", ylab="log2(Total mapped reads)")
points(jitter(x, factor=1.5), c(log2(feature_counts[1:12]+1), log2(feature_counts[13:17]+1)), pch=20, cex=2.5, col=x+1, ylim=c(23,25),xaxt="n", cex.axis=1.5, cex.lab=1.5, main="Rsubread")
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) # this gives us more control over the axis
#dev.off()
...
hist(apply(TCGA_her2_log2,1,sd)/apply(TCGA_her2_log2,1,mean),main = "TCGA Level 3 log",xlab = "Coefficient of variation",ylim=c(0,12500),lwd=4)
...
# ####Coefficient of variation in TCGA pipeline processed data
# hist(apply(log2(TCGA_her2[,13:17]+1),1,sd)/apply(log2(TCGA_her2[,13:17]+1),1,mean),main = "TCGA Level 3\n HER2 samples",xlab = "Coefficient of variation")
# 
# ####Coefficient of variation in Rsubread pipeline processed data
# hist(apply(rsub_fpkmlog[,13:17],1,sd)/apply(rsub_fpkmlog[,13:17],1,mean),main = "Rsubread FPKM \n HER2 samples",xlab = "Coefficient of variation")
# hist(apply(rsub_tpmlog[,13:17],1,sd)/apply(rsub_tpmlog[,13:17],1,mean),main = "Rsubread TPM \n HER2 samples",xlab = "Coefficient of variation")
#dev.off()

###TCGA pipeline normalized ERBB2 expression values
standardized_mean(m.1=mean(tcga_her2_log_normalized$ERBB2[13:17]),sd.1=sd(tcga_her2_log_normalized$ERBB2[13:17]),n.1=5,m.2=mean(tcga_her2_log_normalized$ERBB2[1:12]),sd.2=sd(tcga_her2_log_normalized$ERBB2[1:12]),n.2=12)
...
boxplot(tcga_her2_log_normalized$ERBB2[1:12],tcga_her2_log_normalized$ERBB2[13:17],rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[1:12],rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[13:17],rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[1:12],rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[13:17],ylab="log2(HER2 gene expression values)",range=0,cex.axis=1.5, cex.lab=1.5,outpch=NA,col='grey',medcol="grey",lwd=3)
names=c("TCGA\nGFP","TCGA\nHER2","Rsubred FPKM\nGFP", "Rsubred FPKM\nHER2","Rsubred TPM\nGFP", "Rsubred TPM\nHER2")
text(seq(1,6,by=1),par("usr")[3] - 2.5, labels = names, srt = 45, pos = 1, xpd = TRUE)
points(jitter(rep(1,12)),tcga_her2_log_normalized$ERBB2[1:12],pch=4)
points(jitter(rep(2,5)),tcga_her2_log_normalized$ERBB2[13:17],pch=4)
points(jitter(rep(3,12)),rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[1:12],pch=4)
points(jitter(rep(4,5)),rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[13:17],pch=4)
points(jitter(rep(5,12)),rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[1:12],pch=4)
points(jitter(rep(6,5)),rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[13:17],pch=4)
#dev.off()
...
par(mfrow = c(1, 1))
boxplot(ihc_pos$Rsem_log_q_200_f,ihc_neg$Rsem_log_q_200_f,ihc_pos$FPKM_log_q_200_f,ihc_neg$FPKM_log_q_200_f,ihc_pos$TPM_log_q_200_f,ihc_neg$TPM_log_q_200_f,cex.axis=1.5, cex.lab=1.5,outpch=NA,col='grey',medcol="grey",lwd=3,names=c("TCGA\nHER2(+)","TCGA\nHER2(-)","FPKM\nHER2(+)","FPKM\nHER2(-)","TPM\nHER2(+)","TPM\nHER2(-)"),range=0)
#dev.off()
...
par(mfrow = c(1, 3),lwd=4)
h1<-hist(all_zeros$PANCAN12,xlab='',ylab='',main='',xlim=c(0,8000),ylim=c(0,2000),lwd=4)
#h2<-hist(all_zeros$FPKM,xlab='',ylab='',main='',xlim=c(0,8000),ylim=c(0,2000),lwd=4)
h3<-hist(all_zeros$TPM,xlab='',ylab='',main='',xlim=c(0,8000),ylim=c(0,2000),lwd=4)
