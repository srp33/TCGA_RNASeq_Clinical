...
plot(jitter(x, factor=1.5), c(log2(expected_counts[1:12]), log2(expected_counts[13:17])), pch=20, cex=2.5, col=x+1,ylim=c(23,25), xlab="", ylab="log2(Total mapped reads)", xaxt="n", cex.axis=1.5, cex.lab=1.5, main="TCGA Level 3")
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) # this gives us more control over the axis
plot(jitter(x, factor=1.5), c(log2(feature_counts[1:12]), log2(feature_counts[13:17])), pch=20, cex=2.5, col=x+1, xlab="", ylab="log2(Total mapped reads)", ylim=c(23,25),xaxt="n", cex.axis=1.5, cex.lab=1.5, main="Rsubread")
axis(1, at=1:2, tick=T, labels=c("Control", "HER2"), cex.axis=1.5) # this gives us more control over the axis
#dev.off()
...
par( mfrow = c( 1, 2 ) )
plot(ecdf_all_ex[[1]],xlab=NA, ylab = NA,col="blue",main="TCGA pipeline",ylim=c(0,1),xlim = c(0,20),cex.axis=1.5, cex.lab=1.5,)
...
par(mfrow = c(1, 1))
names=c('TCGA\nGFP','TCGA\nHER2','Rsubread\nGFP', 'Rsubread\nHER2')
boxplot(jitter(log2(rsem_her2$TCGA_ERBB2[1:12])),jitter(log2(rsem_her2$TCGA_ERBB2[13:17])),jitter(log2(rsub_her2$Rsubread_ERBB2[1:12])),jitter(log2(rsub_her2$Rsubread_ERBB2[13:17])),xlab='',ylab="log2(Gene counts)",main=paste('Comparing TGCA and Rsubread Pipelines','\n', 'in Differentiating HER2 Overexpression from Controls',sep=''),cex.axis=0.9, cex.lab=1.5,range = 0)
#text(seq(1,6,by=1),par("usr")[3] - 1, labels = names, srt = 45, pos = 1, xpd = TRUE)
axis(1, at=1:4, tick=T, labels=c("TCGA\nControl", "TCGA\nHER2","Rsubread\nControl", "Rsubread\nHER2"), cex.axis=0.9) 
points(rep(1,12),jitter(log2(rsem_her2$TCGA_ERBB2[1:12]),factor = 1.5),pch=4)
points(rep(2,5),jitter(log2(rsem_her2$TCGA_ERBB2[13:17]),factor=1.5),pch=4)
points(rep(3,12),jitter(log2(rsub_her2$Rsubread_ERBB2[1:12]),factor = 1.5),pch=4)
points(rep(4,5),jitter(log2(rsub_her2$Rsubread_ERBB2[13:17]),factor = 1.5),pch=4)
...
boxplot(log2(tcga_her2_normalized$ERBB2[1:12]+1),log2(tcga_her2_normalized$ERBB2[13:17]+1),rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[1:12],rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[13:17],rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[1:12],rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[13:17],ylab="log2(HER2 gene expression values)",range=0)
names=c("TCGA\nGFP","TCGA\nHER2","Rsubred FPKM\nGFP", "Rsubred FPKM\nHER2","Rsubred TPM\nGFP", "Rsubred TPM\nHER2")
text(seq(1,6,by=1),par("usr")[3] - 2.5, labels = names, srt = 45, pos = 1, xpd = TRUE)
points(rep(1,12),log2(tcga_her2_normalized$ERBB2[1:12]+1),pch=4)
points(rep(2,5),log2(tcga_her2_normalized$ERBB2[13:17]+1),pch=4)
points(rep(3,12),rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[1:12],pch=4)
points(rep(4,5),rsub_fpkmlog_her2$Rsubread_FPKMlog_ERBB2[13:17],pch=4)
points(rep(5,12),rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[1:12],pch=4)
points(rep(6,5),rsub_tpmlog_her2$Rsubread_TPMlog_ERBB2[13:17],pch=4)
#dev.off()
...
```{r}
pancan12_zero<-read.table("~/Desktop/PANCAN12.IlluminaHiSeq_RNASeqV2.geneExp._numZeroes.txt",row.names=1,sep='\t')
pancan20_fpkm_zero<-read.table("~/Desktop/PANCAN20.IlluminaHiSeq_RNASeqV2.tumor_Rsubread_RPKMlog_numZeroes.txt",row.names=1,sep='\t',skip = 1)
pancan20_tpm_zero<-read.table("~/Desktop/PANCAN20.IlluminaHiSeq_RNASeqV2.tumor_Rsubread_TPMlog_numZeroes.txt",sep='\t',row.names=1,skip = 1)
fpkm_tpm<-merge_drop(pancan20_fpkm_zero,pancan20_tpm_zero,by=0)
all_zeros<-merge_drop(pancan12_zero,fpkm_tpm)
#3380 samples are common
hist(all_zeros$TCGA,xlab='',ylab='',main='',xlim=c(0,8000),ylim=c(0,2000))
hist(all_zeros$FPKM,xlab='',ylab='',main='',xlim=c(0,8000),ylim=c(0,2000))
hist(all_zeros$TPM,xlab='',ylab='',main='',xlim=c(0,8000),ylim=c(0,2000))
