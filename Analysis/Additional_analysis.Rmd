...
##identify replicates common in PANCAN12 and our Rsubread processed datasets
find_biological_replicate<-function(matrix){ ##This function identifies TCGA replicates from same patients using first 12 character identifier 'XXXX-XX-XXXX' 
  s=NULL
...
setwd("~/Desktop/")
tcga20_tpm<-data.frame(fread("PANCAN20.IlluminaHiSeq_RNASeqV2.tumor_Rsubread_TPM_10_9.txt"),row.names=1,check.names = F)##Rsubread processed tumor dataset that is TPM processed
dim(tcga20_tpm)
...
rsem<-data.frame(fread("PANCAN12.IlluminaHiSeq_RNASeqV2.geneExp.tumor_whitelist"),row.names=1,check.names = F)###PANCAN12 dataset from Synapse
...
#pdf("Replicates_scatter_plot.pdf")
par( mfrow = c( 4, 4) ,lwd=1)
for(i in 1:13){
 
  plot(biological_rep_12_o[,(i*2-1)],biological_rep_12_o[,(i*2)],xlab=colnames(biological_rep_12_o)[(i*2-1)],ylab=colnames(biological_rep_12_o)[(i*2)])
  ...
  title(paste("TCGA Level 3 \nrho=",round(c$estimate,digits = 2),sep=""))
  print(i)
  print(colnames(biological_rep_12_o)[i])
  plot(biological_rep_20_o[,(i*2-1)],biological_rep_20_o[,(i*2)],xlab=colnames(biological_rep_20_o)[(i*2-1)],ylab=colnames(biological_rep_20_o)[(i*2)])
...
for(i in 7:7){
  ...
  plot(log2(biological_rep_12_o[,(i*2-1)]+1),log2(biological_rep_12_o[,(i*2)]+1),xlim=c(0,20),ylim=c(0,20),xlab=colnames(biological_rep_12_o)[(i*2-1)],ylab=colnames(biological_rep_12_o)[(i*2)])
  c=cor.test(biological_rep_12_o[,(i*2-1)],biological_rep_12_o[,(i*2)],method="spearman")
  cor_res_12=rbind(cor_res_12,c(colnames(biological_rep_12_o)[(i*2-1)],round(total_12[(i*2-1)],digits = 3),colnames(biological_rep_12_o)[(i*2)],round(total_12[(i*2)],digits = 3),round(c$estimate,digits = 3)))
  ...
  print(i)
  print(colnames(biological_rep_12_o)[i])
  plot(log2(biological_rep_20_o[,(i*2-1)]+1),log2(biological_rep_20_o[,(i*2)]+1),xlim=c(0,20),ylim=c(0,20),xlab=colnames(biological_rep_20_o)[(i*2-1)],ylab=colnames(biological_rep_20_o)[(i*2)])
  c<-cor.test(biological_rep_20_o[,(i*2-1)],biological_rep_20_o[,(i*2)],method="spearman")
  cor_res_20=rbind(cor_res_20,c(colnames(biological_rep_20_o)[(i*2-1)],round(total_20[(i*2-1)],digits = 3),colnames(biological_rep_20_o)[(i*2)],round(total_20[(i*2)],digits=3),round(c$estimate,digits = 3)))
...
colnames(cor_res_12)=c("Replicate_1","log2 Level 3 gene counts","Replicate_2","log2 Level 3 gene counts","Spearman correlation between replicates(Level 3)")
colnames(cor_res_20)=c("Replicate_1","log2 Rsubread gene counts","Replicate_2","log2 Rsubread gene counts","Spearman correlation between replicates(Rsubread)")

par( mfrow = c(2, 1 ) ,lwd=4)
hist(as.numeric(cor_res_12[,5]),main = "TCGA Level 3 Two Replicates\n Each for 13 Samples",xlab = "Spearman Correlation ", xlim=c(0.88,1),breaks = 5)
...
cor_res_12
write.table(cbind(cor_res_12,cor_res_20),"Replicates_correlations.txt",sep='\t',col.names=NA,quote=1)
#************************************************************************
####CDF analysis for a pair for biological replicates
...
###Distribution of zero expressing genes across two datasets
...
write.table(prop,"zero_prop.txt",sep='\t',col.names = NA,quote=F)
